...
src/components/admin/    Admin-console display components and typed view-model props.
...
- `src/views/AdminView.vue` is the largest page. Admin navigation, KPI cards, and system health display now live in `src/components/admin/`; next refactor should extract carousel editor, report table, and backup panel.
...
| --- | --- | --- |
| P1 | Admin page | Continue extracting carousel editor, report table, and backup panel until `AdminView.vue` is mostly route orchestration. |
